...
public class RegularJobRunnable implements Runnable {
	...
	public void run() {
		...
		while(true) {
			try {
				if(RegularJobService.find(regularJobEntity.getJobCode()) != null && RegularJobService.find(regularJobEntity.getJobCode()).getStatus() == 1) {
					...
					regularJobEntity.setStatus(RegularJobService.find(regularJobEntity.getJobCode()).getStatus());
